...
# --- HEADER SECTION ---
st.markdown("<h1>MAKER TRENDS — PERFORMANCE DASHBOARD</h1>", unsafe_allow_html=True)
...
if hist_df.empty or vid_df.empty:
    st.warning("Data files not detected. Ensure your data pipeline script runs completely at least once to populate baseline metrics.")
    st.stop()
...
with col1:
    st.markdown(f"<div class='metric-card'><h3>Active Database Date</h3><h2>{latest_date.strftime('%Y-%m-%d')}</h2></div>", unsafe_allow_html=True)
with col2:
    st.markdown(f"<div class='metric-card accent-card'><h3>Top Spiking Phrase</h3><h2>{top_overall_phrase.upper()}</h2></div>", unsafe_allow_html=True)
with col3:
    st.markdown(f"<div class='metric-card'><h3>Analyzed Creators</h3><h2>{vid_df['channel'].nunique():,} Channels</h2></div>", unsafe_allow_html=True)
...
if selected_trend:
    ...
    if not matched_vids.empty:
        ...
        st.info("The selected historical phrase was identified across previous batch iterations, but matching videos are not in the current search snapshot.")
